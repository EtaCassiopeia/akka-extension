...
import akka.io.CustomizedUdp.Bind

/**
  * INTERNAL API
  *
  * CustomizedUdpManager is a facade for simple fire-and-forget style UDP operations.This is a modified version of akka.io.UdpManager class implemented by Typesafe Inc.
  *
  ...
  * @since 2/11/16
  */
class CustomizedUdpManager(udp: CustomizedUdp) extends SelectionHandler.SelectorBasedManager(udp.settings, udp.settings.NrOfSelectors) {

  def receive = workerForCommandHandler {
    case b: Bind =>
      val commander = sender() // cache because we create a function that will run asynchly
      registry => Props(classOf[CustomizedUdpListener], udp, registry, commander, b)
  }
}
